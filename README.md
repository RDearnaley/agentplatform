...
./scripts/conda_init_.zsh                           [run once, creates the conda environmente]
...
TODO: I also provided the conda activate agentplatform command as a script ./scripts/conda.zsh but somehow it doesn't seem to function correctly when run in a script.
...
4. Make Docker build script to add a task
5. Make versions with needed extras preinstalled
6. Improve the web app to include a select-and-run form
7. Add a hash-based Docker image cache mechanism, if Docker doesn't already have one?
