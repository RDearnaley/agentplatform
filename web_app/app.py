...
def build_docker_agent(agent_path, settings):
    _cd_up()
    os.chdir(os.getcwd()+f"/{agent_path}")
    os.system(f'docker build -f Dockerfile.agent --build-arg settings="{settings}" -t "agentplatform/{agent_path}:Dockerfile" --pull=false .')
    _cd_up()
    os.chdir(os.getcwd()+"/web_app")

# Build the task Docker image
def task(agent_path, task_path, settings):
    _cd_up()
    os.chdir(os.getcwd()+f"/{task_path}")
    os.system(f'docker build -f Dockerfile.task --build-arg agent_path="{agent_path}" --build-arg settings="{settings}" -t "agentplatform/{agent_path}/{task_path}:Dockerfile/" --pull=false .')
    _cd_up()
    os.chdir(os.getcwd()+"/web_app")
